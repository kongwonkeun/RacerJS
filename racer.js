...
var game = (function()
{
    ...
    var renderSplashFrame = function()
    {
        ...
        if (keys[32]) { // 32: space
            ...
            gameInterval = setInterval(renderGameFrame, 30); // 1 frame per 30ms
            startTime= new Date();
        }
    }
    
    // renders one frame per 30ms
        ...
        if (absoluteIndex >= roadParam.length - render.depthOfField - 1) {
            ...
            $(window).keydown(function(e) { 
                if (e.keyCode == 84) { // 84: 'T'
                    location.href = "http://twitter.com/home?status=" + escape("I've just raced through #racer10k in " + currentTimeString + "!")
                }
            });
        }
    ...
    var drawSprite = function(sprite)
    {
        ...
        if (h > 0) {
            context.drawImage(spritesheet, sprite.i.x, sprite.i.y, sprite.i.w, h, sprite.x, destY, sprite.s * sprite.i.w, sprite.s * h);
        }
    };
    ...
    var generateRoad = function()
    {
        ...
        while (zones--) {
            ...
            switch (currentStateH) {
                case 0: finalHeight = 0; break;
                case 1: finalHeight =  roadParam.maxHeight * r(); break;
                case 2: finalHeight = -roadParam.maxHeight * r(); break;
            }
            var finalCurve;
            switch (currentStateC) {
                case 0: finalCurve = 0; break;
                case 1: finalCurve = -roadParam.maxCurve * r(); break;
                case 2: finalCurve =  roadParam.maxCurve * r(); break;
            }
            ...
            else currentStateH = transitionH[currentStateH][0];
            if (r() < roadParam.curvy)
                 currentStateC = transitionC[currentStateC][1 + Math.round(r())];
            else currentStateC = transitionC[currentStateC][0];
        }
